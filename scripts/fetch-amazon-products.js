...
const LIMIT = Number(process.env.AMAZON_KEYWORD_LIMIT || 80);
...
function keywordsFromSources() {
  const google = readJson("google-trends.json", { signals: [] });
  const keywordFile = readJson("trend-keywords.json", { keywords: [] });

  const googleKeywords = (google.signals || [])
    .sort((a, b) => number(b.googleTrendScore) - number(a.googleTrendScore))
    .map(s => s.keyword)
    .filter(Boolean);

  const backupKeywords = (keywordFile.keywords || []).filter(Boolean);
  const staticKeywords = (process.env.TREND_KEYWORDS || "")
    .split(",")
    .map(s => s.trim())
    .filter(Boolean);

  const seen = new Set();
  const merged = [...googleKeywords, ...staticKeywords, ...backupKeywords]
    .map(cleanKeyword)
    .filter(k => k && k.split(/\s+/).length <= 5)
    .filter(k => {
  ...
  fs.writeFileSync("amazon-keywords.json", JSON.stringify({
    updatedAt: new Date().toISOString(),
    source: "Google Trends signals + CJ-derived trend-keywords fallback",
    count: merged.length,
    keywords: merged
  }, null, 2));
...
function demandScore(item, index) {
  ...
  const dropshipPriceFit = price > 5 && price < 120 ? 10 : 4;
...
async function main() {
  const keywords = keywordsFromSources();
  const products = [];
  const byKeyword = [];
  const failed = [];

  console.log(`Built ${keywords.length} Amazon keywords.`);
